...
{
  public UserQueryService(IFreeSql freeSql, ISecurityContextAccessor accessor) : base(freeSql, accessor)
  {
  }
  ...
  public async Task<Paging<GetUserPagingResponse>> GetPagingAsync(GetUserPagingRequest request)
  {
    ...
    List<string>? positionUserIds = null;
    if (request.PositionId != null)
    {
      positionUserIds = await GetPositionUserIdTreeAsync(request.PositionId);
    }
    ...
    var result = await QueryableNoTracking
        ...
        .HasWhere(positionUserIds, p => positionUserIds!.Contains(p.Id))
...

  {
    ...
    // 职位
    var positions = await Query<PositionUser>()
        .Where(p => p.UserId == id)
        .ToListAsync(p => p.PositionId, cancellationToken);
  ...
  public async Task<GetUserByUserNameResponse> GetByUserNameAsync(string userName)
  {
    ...
    // 读取职位用户
    var positionIds = await Query<PositionUser>()
        .Where(p => p.UserId == result.Id)
    ...
    {
      var itemIds = await Query<Position>()
          .Where(p => p.ParentPath.Contains(positionId))
          .ToListAsync(p => p.Id);
      if (itemIds.Count > 0)
      {
        positionIdList.AddRange(itemIds);
      }
    }

    // 读取职位角色
    var roleIds2 = Query<PositionRole>()
        .As("c")
        .Where(p => positionIdList.Contains(p.PositionId));
    // 读取用户角色
  ...
  public async Task<List<string?>> GetIdsByRoleIdAsync(string roleId)
  {
    ...
    var positionIds = Query<PositionRole>()
        .Where(p => p.RoleId == roleId);

    var userIds2 = await Query<PositionUser>()
        .Where(p => positionIds.Any(c => c.PositionId == p.PositionId))
        .ToListAsync(p => p.UserId);
