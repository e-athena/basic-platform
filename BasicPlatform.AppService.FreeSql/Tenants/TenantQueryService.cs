...
public class TenantQueryService : QueryServiceBase<Tenant>, ITenantQueryService
{
    ...
    public async Task<GetTenantDetailResponse> GetAsync(string id, CancellationToken cancellationToken = default)
    {
        ...
        // var clientIds = entity.Applications.Select(p => p.AppId).ToList();
        ...
        foreach (var group in applications.GroupBy(p => p.ClientId))
        {
            ...
            var item = entity
                .Applications
                .FirstOrDefault(p => p.AppId == app.ClientId) ?? new TenantApplicationModel
            {
                TenantId = id,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (entity.Applications.Count > 0)
        {
            ...
            if (apps.Count > 0)
            {
                ...
                {
                    var item = apps.FirstOrDefault(p => p.ClientId == model.AppId);
                    if (item == null)
                    {
                        continue;
                    }

                    model.AppId = item.ClientId;
